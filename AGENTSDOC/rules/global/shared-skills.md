...
- 改动 `skills/claude-roundtable/scripts/switch-api.js` 或同 skill 下共享 `scripts/lib/`。
...
- 共享脚本不等于共享说明文档。每个 skill 都可能被单独下载或安装，因此各自的 `SKILL.md` 和 `references/` 必须能独立说明安装、配置、测活、运行和排障，不得要求用户去另一个 skill 查核心用法。
...
- 从版本内部模块：`skills/claude-roundtable/scripts/lib/`。
- `skills/claude-roundtable/scripts/roundtable-lib/` 是 roundtable 专属控制器模块，不属于共享 profile 脚本同步范围。
- 同步纪律：先修改主版本，再同步到从版本；不允许只改其中一份。两份脚本对共享状态目录的读写协议必须保持一致。
- 文档纪律：`claude-orchestrator` 和 `claude-roundtable` 的使用说明都必须自洽完整。可以说明两者共享脚本协议，但不能把 profile 配置、健康探测、独立 settings 或安全要求只写在其中一边。
...
- 健康缓存：`status=ok` 默认复用 5 小时；普通 `--ping <profile>` 和 `--ping-all` 允许复用 TTL 内结果，`--ping <profile> --force` 和 `--refresh-health` 强制重测；`status=down` 不受 TTL 保护。
- 任何自动选择 profile 的控制器必须调用共享健康探测入口，不能只读取 `health.json.status`；否则会把过期 ok 状态误判为可用。
